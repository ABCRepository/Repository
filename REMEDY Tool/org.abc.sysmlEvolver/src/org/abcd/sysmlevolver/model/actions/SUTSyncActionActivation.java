...
public class SUTSyncActionActivation extends ActionActivation {
	...
	public void doAction() {
		...
		for (String pair : pairs) {
			...
			updateFeatureValue(key_value[0].trim(), key_value[1].trim());
			...
			if (output.getName().equals(name)) {
				...
				if (type.getName().equals("Boolean")) {
					...
					try {
						...
						((IntegerValue) value).value = Integer.MAX_VALUE;
					}
	...
	public void updateFeatureValue(String name, String strValue) {
		FeatureValue featureValue = ((ActiveObject) this.getExecutionContext()).getFeature(name);
